...
do
    ...
    10) #ns1,router1,router2,ns2のルーティングテーブルを確認する
        ...
        echo '# Destination(送信先) の 0.0.0.0/0 はすべてのアドレスを表している。'
        echo '# つまりデフォルト・ルートである。デフォルト・ルートとはすべての'
        echo '# ネットワークを集約した経路のことです。'
        echo '# Gateway の 0.0.0.0/24 はNICの自分自身のアドレスを表している。'
        echo '# Genmask はネットマスクを表している。255.255.255.0 は /24である。'
        ;; 
